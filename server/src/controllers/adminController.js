const { createClient } = require('@supabase/supabase-js'); const config = require('../config'); let supabaseAdmin; if (config.supabase.serviceKey) {
    ...
    try {
            ...
            if (role === 'HOD' && departmentCode) {
                ...
                if (currentHOD) {
                    await supabaseAdmin.auth.admin.updateUserById(currentHOD.id, {
                        user_metadata: { ...currentHOD.user_metadata, role: 'LECTURER' },
                        app_metadata: { roles: ['LECTURER'] }
                    });
                    await supabaseAdmin.from('users').update({ roles: ['LECTURER'] }).eq('id', currentHOD.id);
                    // Also update staff table designation if exists
                    await supabaseAdmin.from('staff').update({ designation: 'LECTURER' }).eq('user_id', currentHOD.id);
                }
                ...
                if (currentPrincipal) {
                    await supabaseAdmin.auth.admin.updateUserById(currentPrincipal.id, {
                        user_metadata: { ...currentPrincipal.user_metadata, role: 'LECTURER' },
                        app_metadata: { roles: ['LECTURER'] }
                    });
                    await supabaseAdmin.from('users').update({ roles: ['LECTURER'] }).eq('id', currentPrincipal.id);
                    // Also update staff table designation if exists
                    await supabaseAdmin.from('staff').update({ designation: 'LECTURER' }).eq('user_id', currentPrincipal.id);
                }
            }
        }
        ...
        const newRoles = role === 'SUPER_ADMIN' ? ['SUPER_ADMIN'] : [role, 'LECTURER'];
        ...
        // Update staff designation for the new role as well
        if (role !== 'SUPER_ADMIN') {
            await supabaseAdmin.from('staff').update({ designation: role }).eq('user_id', userId);
        }
...
}; exports.getStaffDashboardData = async (req, res, next) => {
    if (!ensureAdmin(res)) return;
    try {
